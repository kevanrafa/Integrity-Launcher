            ...
            if std::env::var_os("FLATPAK_ID").is_some() {
                _ = main_window.update(cx, |_, window, cx| {
                    window.open_dialog(cx, move |modal, _, _| {
                        let error = "Integrity Launcher was downloaded through Flathub, which has a policy disallowing software from accessing both X11 and Wayland.\n\nMinecraft will be forced to run under Wayland, which may cause issues.\n\nhttps://github.com/flathub-infra/flatpak-builder-lint/pull/935";
    ...
    let handle = cx.open_window(
        WindowOptions {
            ...
            titlebar: Some(TitlebarOptions {
                title: Some("Integrity Launcher (Fork of Pandora)".into()),
