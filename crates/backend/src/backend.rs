...
use crate::{
    account::{BackendAccountInfo, MinecraftLoginInfo}, directories::LauncherDirectories, discord_rpc::{DiscordRpcManager, RpcUiState}, id_slab::IdSlab, instance::{ContentFolder, Instance}, launch::Launcher, metadata::{items::{CurseforgeGetFilesMetadataItem, MinecraftVersionManifestMetadataItem}, manager::MetadataManager}, mod_metadata::ModMetadataManager, persistent::Persistent, server_list_pinger::ServerListPinger, skin_manager::SkinManager
};
...
pub fn start(launcher_dir: PathBuf, send: FrontendHandle, self_handle: BackendHandle, recv: BackendReceiver) {
    ...
    let discord_rpc_config = config.get().discord_rpc.clone();
    ...
    let state = BackendState {
        ...
        discord_rpc: Arc::new(parking_lot::Mutex::new(DiscordRpcManager::new(discord_rpc_config))),
    };

    log::debug!("Doing initial backend load");

    runtime.block_on(async {
        state.discord_rpc.lock().set_ui_state(RpcUiState::IdleInLauncher, None);
...
pub struct BackendState {
    ...
    pub discord_rpc: Arc<parking_lot::Mutex<DiscordRpcManager>>,
}
...
impl BackendState {
    async fn start(self, recv: BackendReceiver, watcher_rx: Receiver<notify_debouncer_full::DebounceEventResult>) {
        ...
        state.clone().handle(recv, watcher_rx).await;
        state.discord_rpc.lock().shutdown();
    }
    ...
    async fn handle_tick(&self) {
        ...
        let mut instance_state = self.instance_state.write();
        let mut any_active_session = false;
    ...
    // Remove launcher-managed hidden mod links from previous launches
        if let Ok(read_dir) = std::fs::read_dir(&mod_dir) {
            for entry in read_dir {
                ...
                if crate::is_managed_mod_hidden_filename(&file_name.to_string_lossy()) {
            ...
            for file in modpack_install.hashed_downloads {
                ...
                if dest_path.starts_with("mods") && dest_path.extension() == Some("jar") {
                    if loader_supports_add_mods {
                        ...
                        let filename = format!("{}{filename}", crate::MANAGED_MOD_PREFIX);
            ...
            if let Some(uuid) = selected_account {
                if let Some(account) = account_info.accounts.get(&uuid) {
                    if account.is_offline() {
                        let access_token = MinecraftAccessToken::new(format!(
                            "offline-token-{}",
                            uuid.as_hyphenated()
                        ));
