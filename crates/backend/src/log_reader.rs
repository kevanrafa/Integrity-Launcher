    ...
    std::thread::spawn(move || {
            ...
            match panic_result {
                ...
                Err(panic_error) => {
                    ...
                    sender.send(MessageToFrontend::AddGameOutput {
                        ...
                        text: Arc::new([format!("(Integrity Launcher) There was an error while reading the log: {panic_error_str}").into()]),
                    });
                    return;
                },
            }
        ...
        if let Err(error) = result {
            sender.send(MessageToFrontend::AddGameOutput {
                ...
                time: Utc::now().timestamp_millis(),
                level: GameOutputLogLevel::Fatal,
                text: Arc::new([format!("(Integrity Launcher) There was an error while reading the log: {error}").into()]),
            });
        }
    });
}
