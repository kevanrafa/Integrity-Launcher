...
impl Launcher {
    ...
    async fn load_mojang_java_binary(
        ...
        let runtime_component_dir = self
            .directories
            .runtime_base_dir
    ...
    fn find_system_java(&self, version_info: &MinecraftVersion, preferred_java_major_version: Option<u32>) -> Option<PathBuf> {
        ...
        if let Some(path) = command::get_command_path("java") {
            candidates.push(path.to_path_buf());
        }

        for candidate in candidates {
            let Some(binary) = Self::search_for_java_binary(&candidate) else {
                continue;
            };
            let Some(major_version) = self.get_major_java_version(&binary) else {
                continue;
    ...
    async fn load_assets(
        ...
        let assets_index_item = AssetsIndexMetadataItem {
        ...
        let assets_index = match meta.fetch(&assets_index_item).await {
            Ok(index) => index,
            Err(err) => {
                log::warn!("Initial asset index fetch failed, retrying with forced reload: {err}");
                tokio::time::sleep(std::time::Duration::from_millis(350)).await;
                let (retry, _) = meta.fetch_with_keepalive(&assets_index_item, true).await;
                retry?
            }
        };
...
pub enum LoadAssetObjectsError {
    ...
    #[error("Downloaded file had the wrong hash")]
    WrongHash,
    #[error("Downloaded file had non-200 status: {0}")]
    NonOkStatus(u16),
    #[error("Asset download failed after retries: {0}")]
    RetryFailed(String),
    #[error("Failed to load metadata:\n{0}")]
    MetaLoadError(#[from] MetaLoadError),
}
...
async fn do_asset_objects_load(
    ...
    for (_, asset) in &assets_index.objects {
        ...
        let fallback_url = format!("https://piston-data.mojang.com/v1/objects/{}", &asset.hash);
        let legacy_url = format!("https://resources.minecraft.net/{}/{}", &asset.hash[..2], &asset.hash);
        ...
        let task = async move {
            ...
            let candidate_urls = [url.as_str(), fallback_url.as_str(), legacy_url.as_str()];
            let mut last_error = String::from("no attempt performed");

            for _ in 0..3 {
                for candidate in candidate_urls {
                    let permit = download_semaphore.acquire().await.unwrap();
                    let response = match http_client.get(candidate).send().await {
                        Ok(response) => response,
                        Err(error) => {
                            drop(permit);
                            last_error = format!("request failed for {candidate}: {error}");
                            continue;
                        }
                    };
                    if !response.status().is_success() {
                        last_error = format!("non-success status {} for {candidate}", response.status());
                        drop(permit);
                        continue;
                    }
                    ...
                    let response_bytes = match response.bytes().await {
                        ...
                        Err(error) => {
                            drop(permit);
                            last_error = format!("unable to read bytes for {candidate}: {error}");
                            continue;
                        }
                    };
                    drop(permit);

                    if response_bytes.len() != asset.size as usize {
                        last_error = format!(
                            "unexpected size for {candidate}, expected {}, got {}",
                            asset.size, response_bytes.len()
                        );
                        continue;
                    }

                    let response_bytes = Arc::new(response_bytes.to_vec());
                    let correct_hash = {
                        let bytes_for_hash = Arc::clone(&response_bytes);
                        tokio::task::spawn_blocking(move || {
                            let mut hasher = Sha1::new();
                    ...
                    if !correct_hash {
                        last_error = format!("hash mismatch for {candidate}");
                        continue;
                    }
            ...
            let Some(bytes) = bytes else {
                return Err(LoadAssetObjectsError::RetryFailed(last_error));
            };
...
impl LaunchContext {
    pub async fn launch(mut self, version_info: &MinecraftVersion) -> std::io::Result<PandoraChild> {
        ...
        let mut child = command.spawn_sandboxed(PandoraSandbox {
            ...
            #[cfg(windows)]
            name: Arc::from(OsStr::new("IntegrityInstanceSandbox")),
            #[cfg(windows)]
            description: Arc::from(OsStr::new("Sandbox for Minecraft instances run by Integrity Launcher")),
            #[cfg(windows)]
    ...
    fn resolve_expansion(&self, key: ArgumentExpansionKey) -> Cow<'_, OsStr> {
        match key {
            ...
            ArgumentExpansionKey::UserType => OsStr::new(if self.login_info.offline { "offline" } else { "msa" }).into(),
